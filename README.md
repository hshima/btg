...
- Linguagem de programação back end: Java + spring boot 
  - Intellij
  - Java 21
  - Spring Boot 3.3.1
  - flyway
- Linguagem de programação da aplicação de testes messageSender: Javascript + nodejs
  - vanilla javascript
  - nvm 1.1.11
  - node 20.15.1
- Banco de Dados: postgres container
  - imagem: postgres:16.3-bullseye
- Tecnologias de Container:
  - Container engine
    - podman: 5.1.1
  - Engine de fila: rabbitmq
    - imagem: rabbitmq:3.13.4

### Organização do trabalho

1. Entendimento da solução e criação do modelo físico de dados
2. Implementação do modelo de entidades no back end
3. Entendimento da tecnologia RabbitMQ
4. Implementação da aplicação de envio de mensagens para o messageSender
5. Criação do método que consome a mensagem e persiste no banco
6. Criação do método que apresenta o valor total do pedido
7. Criação do método que apresenta a quantidade de Pedidos por Cliente
8. Criação do método que apresenta a lista de pedidos realizados por cliente
